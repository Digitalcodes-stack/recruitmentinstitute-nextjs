...
/* â"€â"€â"€ Experts â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€ */
...
/* â"€â"€â"€ General Bot â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€ */
...
/* â"€â"€â"€ Expert personas â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€ */
...
const EXPERT_FN: Record<string, (msg: string) => string> = {
  jhon: (msg) => {
    ...
    /* Recruitment basics / fundamentals */
    ...
    /* Hiring lifecycle / process / steps */
  ...
  riya: (msg) => {
    ...
    /* Fresher / beginner / no experience */
    ...
    /* HR basics / HR fundamentals generally */
  ...
  arjun: (msg) => {
    ...
    /* Boolean / sourcing / LinkedIn / tools */
  ...
  sanjay: (msg) => {
    ...
    /* Fundamentals / recruitment basics */
...
/* â"€â"€â"€ Timer messages â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€ */

const WARN_MSG  = (name: string, rate: string) =>
  `â±ï¸ **Heads up** "" your free 2 minutes with ${name} are almost up (15 seconds left). After this, the session is chargeable at **${rate}**.\n\nBook a course for unlimited expert access.`

const EXPIRE_MSG = (name: string, rate: string) =>
  `ðŸ"’ **Your free session has ended.**\n\nTo continue with ${name}:\n\n"¢ **Continue paid** "" ${rate}\n"¢ **Book a course** "" Unlimited expert access\n"¢ **Call free** "" +91 99750 48884`

/* â"€â"€â"€ Markdown renderer â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€â"€ */
...
export default function ChatAssistant() {
  ...
  return (
    <>
      {/* â"€â"€ Floating stack â"€â"€ */}
      <div style={{ position: 'fixed', bottom: 24, right: 24, zIndex: 1000, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>

        {/* WhatsApp */}
        <a href="https://wa.me/917385204165?text=Hello%2C%20I%20am%20interested%20in%20Recruitment%20Institute%20courses"
          target="_blank" rel="noopener noreferrer" aria-label="WhatsApp" className="ec-wa-btn"
          style={{ width: 50, height: 50, borderRadius: '50%', background: '#25D366', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 8px 24px rgba(37,211,102,0.45)', transition: 'transform 0.2s', textDecoration: 'none' }}>
        ...
        <button onClick={() => setOpen(v => !v)} aria-label="Chat" className="ec-chat-btn"
          style={{ width: 60, height: 60, borderRadius: '50%', background: 'linear-gradient(145deg,#1E3A8A,#1E40AF,#3B5BDB)', color: 'white', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 6px rgba(0,0,0,0.12), 0 10px 32px rgba(30,64,175,0.55)', transition: 'transform 0.22s, box-shadow 0.22s', position: 'relative' }}>
      ...
      {/* â"€â"€ Chat window â"€â"€ */}
      {open && (
        <div style={{ position: 'fixed', bottom: 156, right: 24, zIndex: 999, width: 390, maxWidth: 'calc(100vw - 32px)', maxHeight: 'calc(100vh - 180px)', background: 'white', borderRadius: 22, boxShadow: '0 28px 90px rgba(0,0,0,0.20)', border: '1px solid #E2E8F0', display: 'flex', flexDirection: 'column', overflow: 'hidden', animation: 'ecSlideUp 0.22s ease' }}>

          {/* â"€â"€ Header â"€â"€ */}
          <div style={{ background: 'linear-gradient(135deg,#0F172A,#1E3A8A)', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 10, position: 'relative', overflow: 'hidden', flexShrink: 0 }}>
            ...
            <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, background: 'linear-gradient(90deg,#1E40AF,#6366F1,#0EA5E9)' }} />
            ...
            {showBack && (
              ...
              }} style={{ width: 28, height: 28, borderRadius: 8, background: 'rgba(255,255,255,0.1)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', flexShrink: 0, marginRight: 2 }}>
            ...
            {screen === 'chat' && activeExpert ? (
              <div style={{ width: 36, height: 36, borderRadius: '50%', background: activeExpert.bg, border: `2px solid ${activeExpert.color}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, position: 'relative' }}>
                <span style={{ fontSize: 12, fontWeight: 800, color: activeExpert.color }}>{activeExpert.avatar}</span>
                <span style={{ position: 'absolute', bottom: 0, right: 0, width: 9, height: 9, borderRadius: '50%', background: '#10B981', border: '1.5px solid #1E3A8A' }} />
              </div>
            ) : screen === 'bot' ? (
              ...
              <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg,#1E40AF,#6366F1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <Users style={{ width: 17, height: 17, color: 'white' }} />
              </div>
            )}

            <div style={{ flex: 1 }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: 'white', lineHeight: 1.2 }}>{headerTitle}</p>
              <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.55)', marginTop: 1 }}>{headerSub}</p>
            </div>
            ...
            <button onClick={() => setOpen(false)} style={{ background: 'rgba(255,255,255,0.1)', border: 'none', borderRadius: 8, width: 28, height: 28, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', flexShrink: 0 }}>
          ...
          {screen === 'greeting' && (
            <div style={{ padding: '24px 20px', display: 'flex', flexDirection: 'column', gap: 14, overflowY: 'auto' }}>
              <div style={{ textAlign: 'center' }}>
                <div style={{ width: 60, height: 60, borderRadius: '50%', background: 'linear-gradient(135deg,#EFF6FF,#F5F3FF)', border: '2px solid #DBEAFE', margin: '0 auto 12px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <MessageCircle style={{ width: 26, height: 26, color: '#1E40AF' }} />
                </div>
                <h3 style={{ fontSize: 15, fontWeight: 800, color: '#0F172A', marginBottom: 6 }}>How can we help you?</h3>
                <p style={{ fontSize: 12, color: '#64748B', lineHeight: 1.7 }}>
                  Get instant answers from our AI assistant or connect with a real recruitment expert "" <strong style={{ color: '#1E40AF' }}>first 2 minutes free!</strong>
                </p>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                ...
                <button onClick={openBot} className="ec-option-bot"
                  style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, padding: '16px 12px', borderRadius: 14, background: 'linear-gradient(135deg,#F8FAFC,#EFF6FF)', border: '1.5px solid #BFDBFE', cursor: 'pointer', transition: 'all 0.2s' }}>
                  <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'linear-gradient(135deg,#1E40AF,#6366F1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Bot style={{ width: 18, height: 18, color: 'white' }} />
                  </div>
                  <div style={{ textAlign: 'center' }}>
                    <p style={{ fontSize: 12, fontWeight: 800, color: '#0F172A', lineHeight: 1.2 }}>AI Assistant</p>
                    <p style={{ fontSize: 10, color: '#64748B', marginTop: 3, lineHeight: 1.4 }}>Quick answers about courses, fees & enrollment</p>
                  </div>
                  <span style={{ fontSize: 10, fontWeight: 700, color: '#059669', background: '#F0FDF4', border: '1px solid #BBF7D0', padding: '2px 8px', borderRadius: 50 }}>Instant</span>
                </button>

                {/* Expert option */}
                <button onClick={() => setScreen('need')} className="ec-option-expert"
                  style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, padding: '16px 12px', borderRadius: 14, background: 'linear-gradient(135deg,#F8FAFC,#FFF7ED)', border: '1.5px solid #FDE68A', cursor: 'pointer', transition: 'all 0.2s' }}>
                  ...
                  <div style={{ textAlign: 'center' }}>
                    <p style={{ fontSize: 12, fontWeight: 800, color: '#0F172A', lineHeight: 1.2 }}>Live Expert</p>
                    <p style={{ fontSize: 10, color: '#64748B', marginTop: 3, lineHeight: 1.4 }}>Chat with a real recruitment trainer</p>
                  </div>
                  <span style={{ fontSize: 10, fontWeight: 700, color: '#D97706', background: '#FFFBEB', border: '1px solid #FDE68A', padding: '2px 8px', borderRadius: 50 }}>2 min FREE</span>
                </button>
              </div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, justifyContent: 'center' }}>
                {EXPERTS.slice(0, 3).map(e => (
                  <div key={e.id} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '4px 10px', borderRadius: 50, background: e.bg, border: `1px solid ${e.color}25`, fontSize: 10, fontWeight: 700, color: e.color }}>
                    <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#10B981', display: 'inline-block' }} />
                    {e.name.split(' ')[0]}
                  </div>
                ))}
                <div style={{ padding: '4px 10px', borderRadius: 50, background: '#F8FAFC', border: '1px solid #E2E8F0', fontSize: 10, color: '#64748B' }}>+{EXPERTS.length - 3} more</div>
              </div>

              {/* Quick links */}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                <Link href="/courses" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '9px', borderRadius: 10, background: '#EFF6FF', border: '1px solid #BFDBFE', color: '#1E40AF', fontWeight: 700, fontSize: 11, textDecoration: 'none' }}>View Courses</Link>
                <a href="tel:+917385204165" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '9px', borderRadius: 10, background: '#F0FDF4', border: '1px solid #BBF7D0', color: '#059669', fontWeight: 700, fontSize: 11, textDecoration: 'none' }}>📞 Free Callback</a>
              </div>
            </div>
          )}

          {/* â•â•â•â• BOT CHAT â•â•â•â• */}
          {screen === 'bot' && (
            <>
              <div style={{ flex: 1, overflowY: 'auto', padding: '14px 12px', display: 'flex', flexDirection: 'column', gap: 10, minHeight: 260, maxHeight: 360 }}>
                {messages.map((msg, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start', gap: 8, alignItems: 'flex-end' }}>
                    ...
                    <div style={{ maxWidth: '78%', padding: '10px 13px', borderRadius: msg.role === 'user' ? '16px 16px 4px 16px' : '16px 16px 16px 4px', background: msg.role === 'user' ? 'linear-gradient(135deg,#1E40AF,#2563EB)' : '#F8FAFC', color: msg.role === 'user' ? 'white' : '#0F172A', fontSize: 13, lineHeight: 1.65, border: msg.role === 'bot' ? '1px solid #E2E8F0' : 'none' }}>
                      {renderText(msg.text)}
                    </div>
                    {msg.role === 'user' && (
                ...
                {typing && (
                  <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
                    ...
                    <div style={{ padding: '10px 14px', borderRadius: '16px 16px 16px 4px', background: '#F8FAFC', border: '1px solid #E2E8F0', display: 'flex', gap: 4 }}>
                      {[0,1,2].map(j => <span key={j} style={{ width: 6, height: 6, borderRadius: '50%', background: '#94A3B8', animation: `ecDot 1.2s ease-in-out ${j*0.2}s infinite`, display: 'inline-block' }} />)}
                    </div>
                  </div>
                )}
                <div ref={bottomRef} />
              </div>
              <div style={{ padding: '6px 12px 4px', display: 'flex', gap: 5, overflowX: 'auto', borderTop: '1px solid #F1F5F9', flexShrink: 0 }}>
                {BOT_QR.map(qr => (
                  <button key={qr} onClick={() => sendBot(qr)} style={{ fontSize: 10, fontWeight: 600, padding: '4px 9px', borderRadius: 20, background: '#EFF6FF', color: '#1E40AF', border: '1px solid #BFDBFE', cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}>{qr}</button>
                ))}
              </div>
              <div style={{ padding: '10px 12px 12px', display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0 }}>
                <input ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKey}
                  placeholder="Ask me anything..." style={{ flex: 1, padding: '10px 14px', borderRadius: 12, border: '1px solid #E2E8F0', fontSize: 13, outline: 'none', background: '#F8FAFC', color: '#0F172A' }} />
                <button onClick={handleSend} disabled={!input.trim()} style={{ width: 38, height: 38, borderRadius: 10, background: input.trim() ? 'linear-gradient(135deg,#1E40AF,#2563EB)' : '#F1F5F9', border: 'none', cursor: input.trim() ? 'pointer' : 'default', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          ...
          {/* â•â•â•â• NEED SELECT â•â•â•â• */}
          {screen === 'need' && (
            <div style={{ padding: '16px', overflowY: 'auto', maxHeight: 520 }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: '#0F172A', marginBottom: 3 }}>What brings you here today?</p>
              <p style={{ fontSize: 11, color: '#64748B', marginBottom: 14 }}>Select your goal "" we'll match you with the right expert.</p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {NEEDS.map(need => {
                  const Icon = need.icon
                  return (
                    <button key={need.id} onClick={() => { setSelectedNeed(need.id); setScreen('experts') }} className="ec-need-btn"
                      style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 14px', borderRadius: 12, background: 'white', border: '1.5px solid #E2E8F0', cursor: 'pointer', textAlign: 'left', transition: 'all 0.2s' }}>
                      ...
                      <div>
                        <p style={{ fontSize: 13, fontWeight: 700, color: '#0F172A', lineHeight: 1.2 }}>{need.label}</p>
                        <p style={{ fontSize: 10, color: '#94A3B8', marginTop: 2 }}>{need.desc}</p>
                      </div>
                    </button>
                  )
                })}
          ...
          {screen === 'experts' && (
            <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', maxHeight: 520 }}>
              <div style={{ padding: '12px 16px 10px', borderBottom: '1px solid #F1F5F9', flexShrink: 0 }}>
                <p style={{ fontSize: 13, fontWeight: 700, color: '#0F172A', marginBottom: 2 }}>Experts available for you</p>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ width: 7, height: 7, borderRadius: '50%', background: '#10B981', display: 'inline-block' }} />
                  <p style={{ fontSize: 11, color: '#64748B' }}>First 2 minutes with each expert are completely <strong style={{ color: '#059669' }}>FREE</strong></p>
                </div>
              </div>
              <div style={{ overflowY: 'auto', flex: 1, padding: '10px 14px 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                {filteredExperts.map(expert => (
                  <div key={expert.id} style={{ borderRadius: 14, border: `1.5px solid ${expert.color}22`, overflow: 'hidden', background: 'white', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
                    <div style={{ height: 3, background: `linear-gradient(90deg,${expert.color},${expert.color}70)` }} />
                    <div style={{ padding: '12px 14px' }}>
                      <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 8 }}>
                        <div style={{ width: 40, height: 40, borderRadius: '50%', background: expert.bg, border: `2px solid ${expert.color}35`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, position: 'relative' }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                            <p style={{ fontSize: 13, fontWeight: 800, color: '#0F172A' }}>{expert.name}</p>
                          ...
                          <p style={{ fontSize: 10, color: '#64748B', marginTop: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{expert.title}</p>
                          <p style={{ fontSize: 9, color: '#94A3B8', marginTop: 1 }}>{expert.exp}</p>
                        </div>
                      </div>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 10 }}>
                        {expert.tags.map(t => <span key={t} style={{ padding: '2px 8px', borderRadius: 50, background: expert.bg, color: expert.color, fontSize: 9, fontWeight: 700, border: `1px solid ${expert.color}20` }}>{t}</span>)}
                      </div>
                      <button onClick={() => connectExpert(expert)} className="ec-connect-btn"
                        style={{ width: '100%', padding: '10px', borderRadius: 10, background: `linear-gradient(135deg,${expert.color},${expert.color}CC)`, color: 'white', fontWeight: 700, fontSize: 12, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, boxShadow: `0 4px 14px ${expert.color}35`, transition: 'transform 0.15s' }}>
          ...
          {/* â•â•â•â• EXPERT CHAT â•â•â•â• */}
          {screen === 'chat' && (
            <>
              <div style={{ flex: 1, overflowY: 'auto', padding: '14px 12px', display: 'flex', flexDirection: 'column', gap: 10, minHeight: 260, maxHeight: 360 }}>
                {messages.map((msg, i) => {
                  if (msg.role === 'system') return (
                    <div key={i} style={{ padding: '10px 12px', borderRadius: 12, background: 'linear-gradient(135deg,#FFFBEB,#FEF3C7)', border: '1px solid #FDE68A', fontSize: 12, color: '#92400E', lineHeight: 1.6 }}>
                      {renderText(msg.text)}
                      {expired && (
                        <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 6 }}>
                          <a href="tel:+917385204165" style={{ display: 'block', textAlign: 'center', padding: '9px', borderRadius: 8, background: '#1E40AF', color: 'white', fontWeight: 700, fontSize: 12, textDecoration: 'none' }}>📞 Call Free Now</a>
                          <Link href="/courses" style={{ display: 'block', textAlign: 'center', padding: '9px', borderRadius: 8, background: '#059669', color: 'white', fontWeight: 700, fontSize: 12, textDecoration: 'none' }}>Book a Course</Link>
                          <button onClick={resetToHome} style={{ padding: '9px', borderRadius: 8, background: 'white', color: '#64748B', fontWeight: 600, fontSize: 12, border: '1px solid #E2E8F0', cursor: 'pointer' }}>Back to Home</button>
                        </div>
                      )}
                    </div>
                  )
                  return (
                    <div key={i} style={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start', gap: 8, alignItems: 'flex-end' }}>
                      ...
                      <div style={{ maxWidth: '78%', padding: '10px 13px', borderRadius: msg.role === 'user' ? '16px 16px 4px 16px' : '16px 16px 16px 4px', background: msg.role === 'user' ? 'linear-gradient(135deg,#1E40AF,#6366F1)' : '#F8FAFC', color: msg.role === 'user' ? 'white' : '#0F172A', fontSize: 13, lineHeight: 1.65, border: msg.role === 'bot' ? '1px solid #E2E8F0' : 'none' }}>
                        {renderText(msg.text)}
                      </div>
                      {msg.role === 'user' && (
                ...
                {typing && (
                  <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
                    {activeExpert && (
                      <div style={{ width: 28, height: 28, borderRadius: '50%', background: activeExpert.bg, border: `1.5px solid ${activeExpert.color}30`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <span style={{ fontSize: 9, fontWeight: 800, color: activeExpert.color }}>{activeExpert.avatar}</span>
                      </div>
                    )}
                    <div style={{ padding: '10px 14px', borderRadius: '16px 16px 16px 4px', background: '#F8FAFC', border: '1px solid #E2E8F0', display: 'flex', gap: 4 }}>
              ...
              {!expired && (
                <div style={{ padding: '10px 12px 12px', display: 'flex', gap: 8, alignItems: 'center', borderTop: '1px solid #F1F5F9', flexShrink: 0 }}>
                  <input ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKey}
                    placeholder={`Ask ${activeExpert?.name.split(' ')[0] ?? 'expert'} anything...`}
                    style={{ flex: 1, padding: '10px 14px', borderRadius: 12, border: '1px solid #E2E8F0', fontSize: 13, outline: 'none', background: '#F8FAFC', color: '#0F172A' }} />
                  <button onClick={handleSend} disabled={!input.trim()}
                    style={{ width: 38, height: 38, borderRadius: 10, background: input.trim() ? `linear-gradient(135deg,${activeExpert?.color ?? '#1E40AF'},${activeExpert?.color ?? '#1E40AF'}CC)` : '#F1F5F9', border: 'none', cursor: input.trim() ? 'pointer' : 'default', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <Send style={{ width: 15, height: 15, color: input.trim() ? 'white' : '#CBD5E1' }} />
                  </button>
                </div>
              )}
      ...
      <style>{`
        @keyframes ecSlideUp { from { opacity:0; transform:translateY(16px) } to { opacity:1; transform:translateY(0) } }
        @keyframes ecDot { 0%,60%,100% { transform:translateY(0); opacity:.4 } 30% { transform:translateY(-4px); opacity:1 } }
        .ec-wa-btn:hover { transform:scale(1.1) }
        .ec-chat-btn:hover { transform:scale(1.06); box-shadow:0 6px 8px rgba(0,0,0,0.14),0 14px 40px rgba(30,64,175,0.65) !important }
        .ec-pulse-ring { position:absolute; width:60px; height:60px; border-radius:50%; border:2.5px solid rgba(99,102,241,0.55); animation:ecPulse 2.2s ease-out infinite; pointer-events:none }
        @keyframes ecPulse { 0% { transform:scale(1); opacity:0.7 } 70% { transform:scale(1.55); opacity:0 } 100% { transform:scale(1.55); opacity:0 } }
        .ec-option-bot:hover { border-color:#1E40AF !important; transform:translateY(-2px) }
        .ec-option-expert:hover { border-color:#D97706 !important; transform:translateY(-2px) }
        .ec-need-btn:hover { border-color:#6366F1 !important; background:#F8FAFC !important }
        .ec-connect-btn:hover { transform:translateY(-1px) }
      `}</style>
    </>
  )
}
