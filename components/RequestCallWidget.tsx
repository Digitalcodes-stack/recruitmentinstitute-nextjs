...
import { Phone, PhoneCall, CheckCircle2, AlertCircle, X, Clock, User, Sparkles } from 'lucide-react'
...
export default function RequestCallWidget() {
  ...
  const handleCallPriya = () => {
    if (typeof (window as any).aidtOpenTalk === 'function') {
      ...
      const fab = document.querySelector('.aidt-fab') as HTMLElement
      if (fab) {
        fab.click()
      } else {
        setIsOpen(true)
  ...
  return (
    <>
      ...
      <div className="fixed bottom-6 right-5 sm:right-6 z-50 flex flex-col items-end gap-2.5 pointer-events-auto select-none">
        {/* Row 1 (Top): Micro badge + Call Priya Small Purple Icon */}
