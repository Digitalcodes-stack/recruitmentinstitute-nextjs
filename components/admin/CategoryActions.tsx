...
export default function CategoryActions({ id }: { id: number }) {
  const router = useRouter()
  const [deleting, setDeleting] = useState(false)

  async function handleDelete() {
    if (!confirm('Delete this category? All linked courses and FAQs must be removed first.')) return
    setDeleting(true)
    try {
      const res  = await fetch(`/api/admin/categories/${id}`, { method: 'DELETE' })
      const data = await res.json()
      if (!res.ok) { alert(data.message || 'Cannot delete category'); return }
      router.refresh()
    } finally {
      setDeleting(false)
    }
