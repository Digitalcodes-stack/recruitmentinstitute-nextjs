...
export default function CategoryForm({ category }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    else if (!/^[a-z0-9_]+$/.test(form.slug)) errs.slug = 'Only lowercase letters, numbers, underscores'
    ...
    try {
      ...
      router.push('/admin/courses?tab=categories')
      router.refresh()
    } catch {
  ...
  return (
    <div style={{ maxWidth: 500 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
        <button type="button" onClick={() => router.push('/admin/courses?tab=categories')}
      ...
      <div style={{ background: '#fff', border: '1px solid #e8ecf0', borderRadius: 20, boxShadow: '0 1px 4px rgba(0,0,0,0.04)', padding: 28 }}>
        ...
        <p style={{ fontSize: 13, color: '#94a3b8', marginBottom: 24 }}>
          {isEdit ? 'Update the category name or slug.' : 'Create a new course category.'}
        </p>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          {/* Name */}
          <div>
            <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 6 }}>Category Name *</label>
            <input type="text" value={form.name} onChange={handleName} placeholder="e.g. Corporate Training" style={inp('name')} />
          ...
          <div>
            <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 6 }}>
              Slug *
              <span style={{ fontWeight: 400, color: '#94a3b8', marginLeft: 6 }}>used in URLs — auto-generated from name</span>
            </label>
            <input type="text" value={form.slug} onChange={handleSlug} placeholder="e.g. corporate_training" style={inp('slug')} />
            {errors.slug && <p style={{ fontSize: 11, color: '#ef4444', marginTop: 4 }}>{errors.slug}</p>}
