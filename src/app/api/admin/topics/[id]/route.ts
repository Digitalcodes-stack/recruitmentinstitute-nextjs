...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const validated = topicSchema.partial().safeParse({
    ...body,
    ...(body.chapterId !== undefined && { chapterId: Number(body.chapterId) }),
    ...(body.sortOrder !== undefined && { sortOrder: Number(body.sortOrder) }),
  })
  if (!validated.success)
    return NextResponse.json({ success: false, errors: validated.error.flatten().fieldErrors }, { status: 400 })

  const topic = await prisma.topic.update({ where: { id: parseInt(id) }, data: validated.data })
